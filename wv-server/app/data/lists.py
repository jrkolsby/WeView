...
class List(Base):
    ...
    def toDict(self):
        ...
        return {
            "choices": [c.toDict() for c in self.getChoices()],
            "votes": [v.toDict() for v in self.getVotes()],
            "results": self.getResults(),
            "users": [u.toDict() for u in self.getUsers()],
    ...
    def roundRange(self):
        return list(range(int(2**(math.sqrt(BRACKET_SIZE)-self.theRound)), \
